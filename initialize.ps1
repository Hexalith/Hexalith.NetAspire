...
Write-Verbose "Replacing 'NetAspire' with '$PackageName' in all files..."
Write-Verbose "Also replacing 'NetAspire' with '$($PackageName.ToLower())' in all files..."
...
function ProcessFile {
    ...
    try {
        ...
        if ($content -match "NetAspire") {
            $content = $content -replace "NetAspire", $PackageName
            $hasChanges = $true
        }
        
        if ($content -match "NetAspire") {
            $content = $content -replace "NetAspire", $PackageName.ToLower()
            $hasChanges = $true
        }
...
Rename-ProjectItems -SearchPattern "NetAspire" -Replacement $PackageName -ItemType Directory
Rename-ProjectItems -SearchPattern "NetAspire" -Replacement $PackageName -ItemType File
Rename-ProjectItems -SearchPattern "NetAspire" -Replacement $PackageName.ToLower() -ItemType Directory
Rename-ProjectItems -SearchPattern "NetAspire" -Replacement $PackageName.ToLower() -ItemType File
...
Write-Output "Package name has been changed from 'NetAspire' to '$PackageName'"
Write-Output "Lowercase 'NetAspire' has been changed to '$($PackageName.ToLower())'" 